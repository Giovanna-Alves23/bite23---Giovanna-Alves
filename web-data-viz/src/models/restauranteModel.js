...
// Coloque os mesmos parâmetros aqui. Vá para a var instrucaoSql
function anotar(anotacao, star) {
    console.log("ACESSEI O RESTAURANTE MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", anotacao, star);

    // Insira exatamente a query do banco aqui, lembrando da nomenclatura exata nos valores
    //  e na ordem de inserção dos dados.
    var instrucaoSql = `
        INSERT INTO usuarioRestaurante (fkUsuario, fkRestaurante, anotacao, classificacao) 
        VALUES ('${anotacao}', '${star}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
...
function buscar(categoria, vibe, preferencia) {
    console.log("ACESSEI O RESTAURANTE MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", categoria, vibe, preferencia);

    var instrucaoSql = `
        SELECT nomeRestaurante, categoria, vibe, preferencia FROM restaurante
